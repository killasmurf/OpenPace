...
def test_boston_scientific_import():
    ...
    sample_file = (
        project_root / "tests" / "sample_data" / "boston_scientific_sample.hl7"
    )
    ...
    try:
        transmission = parser.parse_message(
            hl7_content, filename="boston_scientific_sample.hl7"
        )
    ...
    patient = (
        session.query(Patient).filter_by(patient_id=transmission.patient_id).first()
    )
    ...
    for i, (var_name, obs_list) in enumerate(list(obs_by_var.items())[:10]):
        ...
        value = (
            sample_obs.value_numeric
            if sample_obs.value_numeric is not None
            else sample_obs.value_text
        )
    ...
    for var in expected_vars:
        if var in obs_by_var:
            ...
            value = (
                obs.value_numeric if obs.value_numeric is not None else obs.value_text
            )
    ...
    episodes = (
        session.query(ArrhythmiaEpisode)
        .filter_by(transmission_id=transmission.transmission_id)
        .all()
    )
