...
def test_settings_panel_integration():
    ...
    print(
        f"     Device: {query_transmission.device_manufacturer} {query_transmission.device_model}"
    )
    ...
    # Test trend loading
    trends = (
        session.query(LongitudinalTrend).filter_by(patient_id=patient.patient_id).all()
    )
