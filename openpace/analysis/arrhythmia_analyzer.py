...
class ArrhythmiaAnalyzer:
    ...
    def calculate_burden_statistics(
        ...
        """
        if (
            "burden" not in trend.variable_name.lower()
            and "afib" not in trend.variable_name.lower()
        ):
        ...
        classification = ArrhythmiaAnalyzer._classify_burden(values, time_points)

        # Calculate time metrics
        time_metrics = ArrhythmiaAnalyzer._calculate_time_metrics(time_points, values)
    ...
    def _categorize_episodes(
        time_points: List[datetime], values: np.ndarray
    ) -> Dict[str, Any]:
        ...
        for i, (tp, val) in enumerate(zip(time_points, values)):
            episode = {"timestamp": tp.isoformat(), "burden_percent": float(val)}
    ...
    def _classify_burden(
        values: np.ndarray, time_points: List[datetime]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": ArrhythmiaAnalyzer._get_classification_description(
                classification_type, mean_burden
            ),
        }
    ...
    def _calculate_time_metrics(
        ...
        return {
            ...
            "percent_above_low_burden": float(
                days_above_low / total_observations * 100
            ),
            "percent_above_moderate_burden": float(
                days_above_moderate / total_observations * 100
            ),
            "percent_above_high_burden": float(
                days_above_high / total_observations * 100
            ),
        }

    @staticmethod
    def _get_classification_description(
        classification_type: str, mean_burden: float
    ) -> str:
