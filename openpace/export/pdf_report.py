...
class PDFReportGenerator:
    ...
    def generate_report(
        ...
        # Executive summary
        story.extend(
            self._create_executive_summary(patient, transmissions, trends, anonymize)
        )
        ...
        # Lead analysis
        lead_trends = {
            k: v for k, v in trends.items() if k.startswith("lead_impedance")
        }
        if lead_trends:
    ...
    def _create_executive_summary(
        ...
        if transmissions:
            ...
            summary_data = [
                ...
                [
                    "First Transmission:",
                    first_trans.transmission_date.strftime("%Y-%m-%d"),
                ],
                [
                    "Last Transmission:",
                    last_trans.transmission_date.strftime("%Y-%m-%d"),
                ],
        ...
        try:
            ...
            if "error" not in analysis:
                ...
                data = [
                    ...
                    [
                        "Depletion Rate:",
                        f"{abs(analysis['depletion_rate_v_per_year']):.3f} V/year",
                    ],
                    [
                        "Remaining Capacity:",
                        f"{analysis['remaining_capacity_percent']:.1f}%",
                    ],
                    [
                        "Years to ERI:",
                        f"{analysis['years_to_eri']:.1f} years"
            ...
            elements.append(
                Paragraph(f"Error analyzing battery: {str(e)}", self.styles["Normal"])
            )
    ...
    def _create_lead_section(self, lead_trends: Dict[str, LongitudinalTrend]) -> List:
        ...
        elements.append(
            Paragraph("Lead Impedance Status", self.styles["SectionHeading"])
        )
        elements.append(Spacer(1, 0.1 * inch))
        ...
        for lead_name, trend in lead_trends.items():
            try:
                ...
                elements.append(
                    Paragraph(f"<b>{lead_title} Lead</b>", self.styles["Heading3"])
                )
                ...
                data = [
                    ...
                    [
                        "Stability Score:",
                        f"{analysis['stability']['score']:.0f}/100 ({analysis['stability']['rating'].title()})",
                    ],
                ...
                elements.append(
                    Paragraph(
                        f"Error analyzing {lead_name}: {str(e)}", self.styles["Normal"]
                    )
                )
    ...
    def _create_arrhythmia_section(self, trend: LongitudinalTrend) -> List:
        ...
        elements.append(
            Paragraph("Arrhythmia Burden Analysis", self.styles["SectionHeading"])
        )
        elements.append(Spacer(1, 0.1 * inch))
        ...
        try:
            ...
            elements.append(
                Paragraph(
                    f"Error analyzing arrhythmia: {str(e)}", self.styles["Normal"]
                )
            )
